...
const EMPTY_TOTALS = (runId: string): RunTotals => ({
  ...
  employerCost: 0, lopDeduction: 0, bonusPaid: 0, loanRecovered: 0, currency: 'INR', gaps: [],
});
 ...
 */
export async function departmentTotals(runId: string): Promise<DepartmentTotals[]> {
  ...
  try {
    const r = await db.execute(sql`
      SELECT e.department_id::text            AS department_id,
       ...
       GROUP BY e.department_id::text, COALESCE(d.name, '')
       ORDER BY COALESCE(d.name, '') ASC`);
...
export async function monthlyTotals(year: number): Promise<MonthTotals[]> {
  ...
  try {
    const r = await db.execute(sql`
      SELECT pr.month, pr.year, pr.status,
        ...
        LEFT JOIN hr_payslips ps ON ps.payroll_run_id = pr.id
       WHERE pr.year = ${y}
       GROUP BY pr.month, pr.year, pr.status
