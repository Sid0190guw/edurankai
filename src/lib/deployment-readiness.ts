...
export const REQUIREMENTS: readonly EnvRequirement[] = Object.freeze([
  ...
  { name: 'DB_FANOUT_CONCURRENCY', severity: 'optional',
    breaks: 'Nothing to set; defaults to 4, one short of POOL_MAX. How many members of a runFanout() wave may be in flight at once — /portal/employee composes its home screen out of twenty-one reads and used to release all twenty-one at once, which does not run them at once: postgres-js queues the surplus where nothing can see or stop it, and on a cold instance each connection it does open pays an ~810ms handshake that the pooler is measured to swallow about a quarter of the time. Four rather than five deliberately leaves the instance a slot, because the same warm process is serving other requests concurrently and one of them is somebody\'s session gate. Raising it to POOL_MAX lets one page hold every connection the instance has; lowering it to 1 serialises roughly thirty round trips at ~135ms each and is only worth doing to reproduce a queueing problem.' },
