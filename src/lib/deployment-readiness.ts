...
export const REQUIREMENTS: readonly EnvRequirement[] = Object.freeze([
  ...
  { name: 'DB_TRY_MS', severity: 'optional',
    breaks: 'Nothing to set; defaults to 3000ms. The bound on the FIRST attempt inside withDbRetry(), which is what the five session/admin/permission/face gates in src/middleware.ts use instead of a single DB_TIMEOUT_MS wait. It is deliberately far shorter than 8000ms: measured on the live site, a read on an instance that already has a connection answers in 132-154ms and one that has to open a connection answers in ~950ms, so anything past three seconds is not slow, it is a connection that is not coming. Raising it back toward DB_TIMEOUT_MS re-creates the failure this replaced -- one stalled handshake rendering the full-page "we cannot reach the database" refusal over a database answering in 132ms.' },
  { name: 'DB_RETRY_MS', severity: 'optional',
    breaks: 'Nothing to set; defaults to 4000ms. The bound on the SECOND attempt inside withDbRetry(). The retry exists because roughly a quarter of the attempts to OPEN a connection on this deployment stall past five seconds while every read that reuses one succeeds -- a fixed rate per connection attempt, not a load-proportional one, which is why four rounds of tuning postgres-js `max` never moved it. The second ask is a second independent draw, and it is cheap because the connection the abandoned first attempt was opening still lands in the pool. DB_TRY_MS + DB_RETRY_MS is the worst case one gate can cost, and the circuit breaker caps how many gates pay it before the rest are refused instantly.' },
