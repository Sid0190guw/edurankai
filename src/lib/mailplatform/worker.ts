...
//
//   1. THE SCHEDULER. Emit the events for scheduled triggers whose instant has passed. Their event
//      ids are DETERMINISTIC — workflow, version, instant, contact — so a tick that runs twice, or
//      two ticks that overlap, produce the same ids and the router's idempotency gate discards the
//      repeats. That is why there is no "already fired" column to get out of step with reality.
//
//   2. THE RUNS. Claim every run whose wait_until has passed, plus any left RUNNING by a worker that
//      died, and advance each one. Claiming is a single atomic statement in the Postgres store, so
//      two overlapping ticks cannot both take the same run.
//
// A tick is bounded, and it says what it did NOT get to. An operator reading "advanced 100 of 340
// due" knows to run it again or raise the cron frequency; a tick that silently processed the first
...
export interface TickReport {
  ...
  /** True when the limit was hit and there is more waiting. Say so; do not imply the queue is empty. */
  moreDue: boolean;
  errors: string[];
}

/** Deterministic, and therefore the whole idempotency story for scheduled fan-out. */
export function scheduleEventId(workflowId: string, version: number, at: string, contactId: string): string {
  return 'sched_' + workflowId + '_v' + version + '_' + Date.parse(at) + '_' + contactId;
}
...
export async function runScheduler(deps: EngineDeps, orgId: string): Promise<{ emitted: number; errors: string[] }> {
  ...
  for (const type of ['schedule.date', 'schedule.time']) {
    const workflows = await store.workflowsListeningFor(orgId, type);
    for (const wf of workflows) {
      const trigger = wf.definition?.nodes?.find((n) => n.kind === 'trigger')?.trigger;
      const at = trigger?.schedule?.at;
      if (!at) {
        errors.push('"' + wf.name + '" is scheduled but names no date, so it will never fire.');
        continue;
      }
      const atMs = Date.parse(at);
      if (!Number.isFinite(atMs)) { errors.push('"' + wf.name + '" has an unreadable scheduled date: ' + at); continue; }
      if (atMs > now.getTime()) continue;   // not yet

      let contacts;
      try {
        contacts = await store.listContacts(orgId, { listKey: trigger?.schedule?.listKey, tag: trigger?.schedule?.tag, limit: SCHEDULE_FANOUT_LIMIT });
      } catch (e: any) { errors.push('"' + wf.name + '": the audience could not be read: ' + reasonOf(e)); continue; }
      ...
      for (const c of contacts) {
        const r = await ingestEvent(
          ...
          makeEvent({
            ...
            eventId: scheduleEventId(wf.id, wf.version, at, c.id),
            payload: { scheduled_for: new Date(atMs).toISOString(), workflow_key: wf.key },
          }),
        );
        if (!r.duplicate && r.accepted) emitted += 1;
        if (r.error) errors.push('"' + wf.name + '": ' + r.error);
      }
    }
  }
...
export async function tick(deps: EngineDeps, opts: { orgId: string; limit?: number } = { orgId: '' }): Promise<TickReport> {
  const store = deps.store;
  const now = (deps.now || (() => new Date()))();
  ...
  for (const run of due) {
    try {
      ...
      report.errors.push('Run ' + run.runId + ' could not be advanced: ' + reason);
      console.error('[mailplatform/worker] run ' + run.runId + ':', reason);
    }
  }
  return report;
}
...
// =================================================================================================
//
// Each of these reports whether it CHANGED anything. A control that always says "paused" is a
// control an operator cannot trust: a run that had already completed, or that another tab cancelled
// a second earlier, must not be reported as paused when it is not.
...
export async function controlRun(store: AutomationStore, orgId: string, runId: string, action: ControlAction): Promise<ControlResult> {
  ...
  if (action === 'pause') {
    ...
    // waitUntil is KEPT. Pausing a run that was waiting until Friday and resuming it on Thursday
    // must not send Friday's mail on Thursday.
  ...
  if (action === 'resume') {
    ...
    // workers could hold the same run. A null waitUntil means "due now".
  ...
  if (action === 'cancel') {
    ...
    return { ok: true, changed: true, state: 'CANCELLED', message: 'Cancelled. Nothing further will be sent for this contact from this workflow.' };
  }
  ...
  // nothing. A step already 'done' is left alone by resetStep, so this cannot repeat a finished send.
