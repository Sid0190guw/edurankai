// POST /api/admin/fee-waivers/reply
// Admin-side reply on a fee waiver thread.
...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  const r = rows(await db.execute(sql`SELECT id, user_id FROM application_fee_waivers WHERE id = ${id} LIMIT 1`));
  if (!r[0]) return json({ ok: false, error: 'not found' }, 404);

  await postMessage({
    requestType: 'fee_waiver',
    requestId: id,
    applicantUserId: r[0].user_id,
    senderRole: 'admin',
