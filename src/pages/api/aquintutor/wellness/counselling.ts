// POST /api/aquintutor/wellness/counselling â€” ask for a counselling appointment.
//
// =================================================================================================
...
//      display name is dropped at this boundary rather than written and filtered later â€” a row that
...
// the organisation", not "unreachable" â€” the page promises a counsellor will confirm, and a request
...
export const POST: APIRoute = async ({ request, clientAddress }) => {
  ...
  if (await tooFast('counselling', clientAddress || contact)) return json({ ok: false, error: 'that went through several times just now â€” give it a minute' }, 429);
  ...
  try {
    ...
    // The reason is logged, never returned â€” an error string from this table could carry the value of
