// POST /api/runtime/observe — Block 04: ingest one attempt, run the BKT update, return the
// updated concept mastery + predicted correctness + cognitive load. Owner-gated.
import type { APIRoute } from 'astro';
import { createPgKernel } from '@/lib/kernel';
import { applyObservation } from '@/lib/runtime/estimators';
import { observeBodySchema } from '@/lib/runtime/estimators/schema';
...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  const parsed = observeBodySchema.safeParse(body);
  ...
  const result = await applyObservation(kernel, parsed.data.studentObjectId, parsed.data.observation as any);
